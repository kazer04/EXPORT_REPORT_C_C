 ...
 * @author User
 */
public class PicFile extends files implements Ifile {

    public PicFile() {
        this.setXtention(".jpg");
    }

    public void InitFiles() {
        this.setExtention(enumExtentionFiles.JPG);
    }
    ...
    public boolean isExiste(String path) {
        try {
            String temp = jdom.path_photo_absolute + path;
            // new logger().OCategory.info(temp);
            InputStream ips = new FileInputStream(temp);
            InputStreamReader ipsr = new InputStreamReader(ips);
            BufferedReader br = new BufferedReader(ipsr);
            this.setPath_outut(jdom.path_photo_relatif + path);
        } catch (Exception e) {
    ...
    public boolean isExisteAbs(String path) {
        try {
            String temp = jdom.path_photo_absolute + path;
            //new logger().OCategory.info(temp);
            InputStream ips = new FileInputStream(temp);
            InputStreamReader ipsr = new InputStreamReader(ips);
            BufferedReader br = new BufferedReader(ipsr);
            this.setPath_outut(jdom.path_photo_absolute + path);
        } catch (Exception e) {
        }
        return false;
    }
